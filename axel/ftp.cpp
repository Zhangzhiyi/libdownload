...
{
	conn->data_fd = -1;
	conn->message = (char*)malloc( MAX_STRING );
...
int ftp_wait( ftp_t *conn )
{
	...
	conn->message = (char*)realloc( conn->message, size );
	...
	if( ( s = strchr( conn->message, '\r' ) ) != NULL )
		*s = 0;
	conn->message = (char*)realloc( conn->message, __max( strlen( conn->message ) + 1, MAX_STRING ) );
