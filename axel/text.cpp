...
void set_verbose(int ver)
{
    g_verbose = ver;
}
extern void update_percent(int taskId, int max, int cur, double percent, double rate);
...
static void print_alternate_output(axel_t *axel)
{
	...
	DebugMsg("\r[%3ld%%] [", __min(100,(long)(done*100./total+.5) ) );
	...
	if(axel->bytes_per_second > 1048576)
		DebugMsg( "] [%6.1fMB/s]", (double) axel->bytes_per_second / (1024*1024) );
	else if(axel->bytes_per_second > 1024)
		DebugMsg( "] [%6.1fKB/s]", (double) axel->bytes_per_second / 1024 );
	else
		DebugMsg( "] [%6.1fB/s]", (double) axel->bytes_per_second );
	...
	if(done<total)
	{
		...
		if(days)
			DebugMsg(" [%2dd%2d]",days,hours);
		else if(hours)
			DebugMsg(" [%2dh%02d]",hours,minutes);
		else
			DebugMsg(" [%02d:%02d]",minutes,seconds);
	}
