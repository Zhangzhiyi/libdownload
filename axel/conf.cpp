...
int conf_init( conf_t *conf )
{
	...
	*conf->no_proxy			= 0;
	...
	conf->interfaces = (message_t*)malloc( sizeof( if_t ) );
...
int parse_interfaces( conf_t *conf, char *s )
{
	...
	iface = (message_t*)conf->interfaces->next;
	while( iface != conf->interfaces )
	{
		if_t *i;
		
		i = (message_t*)iface->next;
		free( iface );
		iface = i;
	}
	...
	s[strlen(s)+1] = 0;
	conf->interfaces = iface = (message_t*)malloc( sizeof( if_t ) );
	while( 1 )
	{
		while( ( *s == ' ' || *s == '\t' ) && *s ) s ++;
