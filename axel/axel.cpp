...
axel_t *axel_new( conf_t *conf, int count, void *url )
{
	...
	*axel->conf = *conf;
	...
	if( buffer == NULL )
		buffer = (char*)malloc( __max( MAX_STRING, axel->conf->buffer_size ) );
	...
	{
		res = (search_t *) url;
		u = axel->url = (url_t*)malloc( sizeof( url_t ) );
		for( i = 0; i < count; i ++ )
		{
...
int axel_open( axel_t *axel )
{
	...
	snprintf( buffer, MAX_STRING, "%s/%s.st", axel->conf[0].saving_path, axel->filename );
	...
	if( !axel->conn[0].supported )
	{
		...
		axel->conn = (conn_t*)realloc( axel->conn, sizeof( conn_t ) );
		axel_divide( axel );
	}
	else if( ( fd = open( buffer, O_RDONLY ) ) != -1 )
	{
		...
		axel->conn = (conn_t*)realloc( axel->conn, sizeof( conn_t ) * axel->conf->num_connections );
        ...
        snprintf( szFullPath, MAX_STRING, "%s/%s", axel->conf[0].saving_path, axel->filename );
		if( ( axel->outfd = open(szFullPath, O_WRONLY, 0666 ) ) == -1 )
		{
	...
	if( axel->outfd == -1 )
	{
		axel_divide( axel );
		char szFullPath[512] = {0};
        snprintf(szFullPath, sizeof(szFullPath), "%s/%s", axel->conf[0].saving_path, axel->filename);
		if( ( axel->outfd = open( szFullPath, O_CREAT | O_WRONLY, 0666 ) ) == -1 )
		{
			axel_message( axel, _("Error opening local file") );
			return( 0 );
		}
		...
		if( lseek( axel->outfd, axel->size, SEEK_SET ) == -1 && axel->conf->num_connections > 1 )
		{
			...
			while( j > 0 )
			{
				write( axel->outfd, buffer, __min( j, axel->conf->buffer_size ) );
...
void axel_do( axel_t *axel )
{
	...
	for( i = 0; i < axel->conf->num_connections; i ++ )
	{
		...
		hifd = __max( hifd, axel->conn[i].fd );
	}
	if( hifd == 0 )
	{
...
#ifdef WIN32
		if ( axel->conn[i].setup_thread[0].p != 0 )
			pthread_cancel( axel->conn[i].setup_thread[0] );
#elif defined __APPLE__
		if ( axel->conn[i].setup_thread[0] != 0 )
			pthread_cancel( axel->conn[i].setup_thread[0] );
#endif
...
double gettime()
{
	...
	if (time)
	{
		...
		time->tv_sec     = (long)( liTime.QuadPart / liFreq.QuadPart );
		time->tv_usec    = (long)( liTime.QuadPart * 1000000.0 / liFreq.QuadPart - time->tv_sec * 1000000.0 );
	}
...
void save_state( axel_t *axel )
{
	...
	snprintf( fn, MAX_STRING, "%s/%s.st", axel->conf[0].saving_path, axel->filename );
...
static void axel_message( axel_t *axel, const char *format, ... )
{
	message_t *m = (message_t*)malloc( sizeof( message_t ) ), *n = axel->message;
	va_list params;
